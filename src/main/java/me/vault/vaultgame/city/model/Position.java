...
/**
 * The {@code Record "Position"} represents the position of an element on the screen and holds an x- and y-coordinate. The x- and y-coordinate are
 * saved as integers and don't allow floating point numbers. They're primarily used by the {@code Building}-enumeration, since the location/position
...
public record Position(int xPos, int yPos)
{
	 ...
	 *
	 * @return Returns {@code true}, if the compared object is an instance of {@link Position} and its attribute values are the same.
	 */
	@Override
