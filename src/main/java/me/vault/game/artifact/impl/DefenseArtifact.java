...
import static me.vault.game.utility.constant.ArtifactConstants.Defense;
 ...
 * <br>
 * As this specification is a defense artifact, it mainly provides positive attribute modifiers towards the damage
 * attribute.
 * Other attribute modifiers might be affected as well by the defense attribute, the highest multiplier is for the
...
public final class DefenseArtifact extends Artifact
{
	...
	static
	{
		// TODO: Einzelne Methoden anstelle von einem riesigen Block.

		// TODO: More specific logs, printing what data was filled into these maps. Not only in this artifact
		//  implementation, but in all artifact implementations.
		LOGGER.logf(DEBUG, NewLoggingConstants.STATIC_INITIALIZER_ENTERED, DefenseArtifact.class.getSimpleName());


		// Fill the map with the different upgrade cost transactions.
		UPGRADE_COSTS.put(ArtifactLevel.BASE, Defense.BASE_TO_IMPROVED_UPGRADE_COSTS);
		UPGRADE_COSTS.put(ArtifactLevel.IMPROVED, Defense.IMPROVED_TO_NONE_UPGRADE_COSTS);


		// Fill the map with the names.
		NAMES.put(ArtifactLevel.BASE, Defense.BASE_NAME);
		NAMES.put(ArtifactLevel.IMPROVED, Defense.IMPROVED_NAME);


		// Fill the map with the sprites.
		SPRITES.put(ArtifactLevel.BASE, ResourceLoader.loadImage(Defense.BASE_SPRITE_PATH));
		SPRITES.put(ArtifactLevel.IMPROVED, ResourceLoader.loadImage(Defense.IMPROVED_SPRITE_PATH));


		// Create and fill the map with modifiers for the BASE artifact level.
		final Map<Type, Double> baseLevelModifiers = new HashMap<>();
		baseLevelModifiers.put(Type.DAMAGE, ArtifactConstants.MULTIPLIER_MINIMUM);
		baseLevelModifiers.put(Type.DEFENSE, ArtifactConstants.MULTIPLIER_HIGH);
		baseLevelModifiers.put(Type.HEALTH, ArtifactConstants.MULTIPLIER_DEFAULT);
		...
		improvedLevelModifiers.put(Type.DAMAGE, ArtifactConstants.MULTIPLIER_LOW);
		improvedLevelModifiers.put(Type.DEFENSE, ArtifactConstants.MULTIPLIER_MAXIMUM);
		improvedLevelModifiers.put(Type.HEALTH, ArtifactConstants.MULTIPLIER_DEFAULT);
		...
		LOGGER.logf(DEBUG, NewLoggingConstants.STATIC_INITIALIZER_LEFT, DefenseArtifact.class.getSimpleName());
	}
