 ...
 * applied to all players or troops that are eligible to receive buffs and de-buffs.
 * <br>
...
public class AttributeMultiplier
{
	...
	/**
	 * This property is used to store and dynamically display the damage multiplier the artifact provides.
	 * If the value is updated within this property, JavaFX instantly applies the change, so it's visible in the GUI.
	 *
	...
	/**
	 * This property is used to store and dynamically display the health multiplier the artifact provides.
	 * If the value is updated within this property, JavaFX instantly applies the change, so it's visible in the GUI.
	 *
	 * @see SimpleDoubleProperty
	 */
	...
	/**
	 * This property is used to store and dynamically display the defense multiplier the artifact provides.
	 * If the value is updated within this property, JavaFX instantly applies the change, so it's visible in the GUI.
	 *
	 * @see SimpleDoubleProperty
	 */
	 ...
	 * <br>
	 * The instance is only created if the supplied modifiers were valid.
	 * Check the {@link AttributeMultiplier#validate()} method for more information.
	 *
	 ...
	 * <br>
	 * Checks if the health multiplier isn't equal or below zero, because negative health or
	 * defense wouldn't make any sense.
	 * Zero health multiplier wouldn't make any sense either, as the unit would instantly be dead in that case, as
	 * the health would always be equal to zero if you multiply it with zero.
	 * <br>
	 * The damage and defense multiplier is validated as well: this method checks if these aren't below zero, because
	 * negative damage or defense values wouldn't make any sense.
	...
	/**
	 * Returns the property which is wrapped around the damage multiplier.
	 * This property needs to be bound to the corresponding GUI element in order for JavaFX to be able to update the
	 * data in the GUI element automatically.
	 *
	...
	/**
	 * Returns the property which is wrapped around the health multiplier.
	 * This property needs to be bound to the corresponding GUI element in order for JavaFX to be able to update the
	 * data in the GUI element automatically.
	 *
	...
	/**
	 * Returns the property which is wrapped around the defense multiplier.
	 * This property needs to be bound to the corresponding GUI element in order for JavaFX to be able to update the
	 * data in the GUI element automatically.
	 *
